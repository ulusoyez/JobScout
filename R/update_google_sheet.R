...
has_filtered_jobs <- is.data.frame(filtered_jobs) &&
  nrow(filtered_jobs) > 0
...
if (nrow(existing_recent) == 0) {
  ...
  if (length(current_state) != 1) {
    stop("Recent jobs must contain exactly one state per JobScout run.")}
...
if (!has_filtered_jobs) {
  new_jobs <- NULL
} else if (nrow(existing_filtered) == 0) {
...
new_job_count <- if (is.null(new_jobs)) 0 else nrow(new_jobs)
...
if (new_job_count > 0) {
  ...
  cat(new_job_count, "new jobs added.\n")
} else {
...
filtered_job_count <- if (has_filtered_jobs) nrow(filtered_jobs) else 0
...
cat(
  ...
  nrow(existing_filtered) + new_job_count,
  "\n",
  sep = "")
